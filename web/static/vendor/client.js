...
Elm.Client.make = function (_elm) {
   ...
   var transport = Elm.Native.Port.make(_elm).outboundSignal("transport",function (v) {    return v;},outbox.signal);
   var log = Elm.Native.Port.make(_elm).inboundSignal("log",
   "String",
   function (v) {
      return typeof v === "string" || typeof v === "object" && v instanceof String ? v : _U.badPort("a string",v);
   });
   var inbox = Elm.Native.Port.make(_elm).inboundSignal("inbox",
   "String",
   function (v) {
      return typeof v === "string" || typeof v === "object" && v instanceof String ? v : _U.badPort("a string",v);
   });
   var connected = Elm.Native.Port.make(_elm).inboundSignal("connected",
   "Bool",
   function (v) {
      return typeof v === "boolean" ? v : _U.badPort("a boolean (true or false)",v);
   });
   var init = {ctor: "_Tuple2",_0: {connected: false,draft: "",history: _U.list([])},_1: $Effects.none};
   var historyView = function (history) {
      return A3($List.foldl,
      F2(function (msg,acc) {
         return _U.eq("__system__",msg.from) ? A2($List._op["::"],
         A2($Html.div,_U.list([$Html$Attributes.$class("alert")]),_U.list([$Html.text(msg.contents)])),
         acc) : _U.eq("__server__",msg.from) ? A2($List._op["::"],
         A2($Html.div,_U.list([$Html$Attributes.$class("alert alert-info text-right")]),_U.list([$Html.text(msg.contents)])),
         acc) : A2($List._op["::"],A2($Html.div,_U.list([$Html$Attributes.$class("alert alert-success text-left")]),_U.list([$Html.text(msg.contents)])),acc);
      }),
      _U.list([]),
      history);
   };
   var connectedView = function (isConnected) {
      return isConnected ? _U.list([A2($Html.span,_U.list([$Html$Attributes.$class("glyphicon glyphicon-ok")]),_U.list([]))]) : _U.list([A2($Html.span,
   ...
   var scrollableStyle = $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "height",_1: "400px"}
                                                        ,{ctor: "_Tuple2",_0: "overflow-y",_1: "auto"}
                                                        ,{ctor: "_Tuple2",_0: "background-color",_1: "white"}]));
   var Log = function (a) {    return {ctor: "Log",_0: a};};
   var Receive = function (a) {    return {ctor: "Receive",_0: a};};
   var Send = function (a) {    return {ctor: "Send",_0: a};};
   var UpdateDraft = function (a) {    return {ctor: "UpdateDraft",_0: a};};
   var UpdateLinkStatus = function (a) {    return {ctor: "UpdateLinkStatus",_0: a};};
   var inputs = $Signal.mergeMany(_U.list([A2($Signal.map,UpdateLinkStatus,connected),A2($Signal.map,Receive,inbox),A2($Signal.map,Log,log)]));
   var NoOp = {ctor: "NoOp"};
   var update = F2(function (action,model) {
      var _p0 = action;
      switch (_p0.ctor)
      {case "UpdateLinkStatus": return {ctor: "_Tuple2",_0: _U.update(model,{connected: _p0._0}),_1: $Effects.none};
         case "UpdateDraft": return {ctor: "_Tuple2",_0: _U.update(model,{draft: _p0._0}),_1: $Effects.none};
         case "Send": if (_p0._0 === "") {
                 return {ctor: "_Tuple2",_0: model,_1: $Effects.none};
              } else {
                 var _p2 = _p0._0;
                 var message = {from: "__client__",contents: _p2};
                 var history = A2($List._op["::"],message,model.history);
                 var task = A2($Task.andThen,A2($Signal.send,outbox.address,_p2),function (_p1) {    return $Task.succeed(NoOp);});
                 return {ctor: "_Tuple2",_0: _U.update(model,{history: history,draft: ""}),_1: $Effects.task(task)};
              }
         case "Receive": var message = {from: "__server__",contents: _p0._0};
           var history = A2($List._op["::"],message,model.history);
           return {ctor: "_Tuple2",_0: _U.update(model,{history: history}),_1: $Effects.none};
         case "Log": var message = {from: "__system__",contents: _p0._0};
           var history = A2($List._op["::"],message,model.history);
           return {ctor: "_Tuple2",_0: _U.update(model,{history: history}),_1: $Effects.none};
         default: return {ctor: "_Tuple2",_0: model,_1: $Effects.none};}
   });
   var onReturn = F2(function (address,action) {
      return A3($Html$Events.on,
      "keypress",
      $Html$Events.keyCode,
      function (code) {
         return _U.eq(13,code) ? A2($Signal.message,address,action) : A2($Signal.message,address,NoOp);
      });
   });
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.id("container")]),
      _U.list([A2($Html.div,_U.list([$Html$Attributes.id("history"),$Html$Attributes.$class("well"),scrollableStyle]),historyView(model.history))
              ,A2($Html.div,
              _U.list([$Html$Attributes.$class("form-group input-group")]),
              _U.list([A2($Html.span,_U.list([$Html$Attributes.$class("input-group-addon")]),connectedView(model.connected))
                      ,A2($Html.input,
                      _U.list([$Html$Attributes.type$("text")
                              ,$Html$Attributes.placeholder("Enter your message...")
                              ,$Html$Attributes.$class("form-control")
                              ,$Html$Attributes.value(model.draft)
                              ,A2(onInput,address,UpdateDraft)
                              ,A2(onReturn,address,Send(model.draft))]),
                      _U.list([]))
                      ,A2($Html.div,
                      _U.list([$Html$Attributes.$class("input-group-btn")]),
                      _U.list([A2($Html.button,
                              _U.list([$Html$Attributes.$class("btn btn-primary")
                                      ,$Html$Attributes.disabled($Basics.not(model.connected))
                                      ,A2($Html$Events.onClick,address,Send(model.draft))]),
                              ...
                              _U.list([A2($Html.span,_U.list([$Html$Attributes.$class("caret")]),_U.list([]))
                                      ,A2($Html.span,_U.list([$Html$Attributes.$class("sr-only")]),_U.list([$Html.text("Toggle Dropdown")]))]))
                              ,A2($Html.ul,
                              _U.list([$Html$Attributes.$class("dropdown-menu dropdown-menu-right")
                                      ,A2($Html$Attributes.attribute,"aria-haspopup","true")
                                      ,A2($Html$Attributes.attribute,"aria-expanded","false")]),
                              _U.list([A2($Html.li,
                              _U.list([]),
                              _U.list([A2($Html.a,_U.list([$Html$Attributes.href("#")]),_U.list([$Html.text("Image or photo")]))]))]))]))]))]));
   });
   var app = $StartApp.start({init: init,inputs: _U.list([inputs]),update: update,view: view});
   var main = app.html;
   var tasks = Elm.Native.Task.make(_elm).performSignal("tasks",app.tasks);
   var Model = F3(function (a,b,c) {    return {connected: a,draft: b,history: c};});
   var Message = F2(function (a,b) {    return {from: a,contents: b};});
